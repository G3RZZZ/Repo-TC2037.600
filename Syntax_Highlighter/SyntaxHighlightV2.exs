...
defmodule Highlighter do
    ...
    # Function that reads a file and makes it one string, removing the \n token
    # aswell.
    def syntaxHighlight(in_filename) do
        ...
        code = identify(code)
        code = String.replace(code, "newline", "\n")
        createHtml(code)
    end

    # Function that accepts the stringified file and identifies the json tokens
    # inside it. It does this by placing already identified tokens in a list
    ...
    def identify(code), do: identify_tail(code, [])
        ...
        defp identify_tail(code, newLine) do
            cond do
                ...
                String.match?(code, ~r{^\s+|^newline}) ->
                    match = Regex.run(~r{(^\s+|^newline)(.*)}, code)
                ...
                String.match?(code, ~r/^-?\d+[.]?\d*[Ee]?[+-]?\d*(?=\s|,|]|}|newline)/) ->
                    match = Regex.run(~r/(^-?\d+[.]?\d*[Ee]?[+-]?\d*(?=\s|,|]|}|newline))(.*)/, code)
                    identify_tail(Enum.at(match, 2), [htmlTag(Enum.at(match, 1),
                    "number") | newLine])
